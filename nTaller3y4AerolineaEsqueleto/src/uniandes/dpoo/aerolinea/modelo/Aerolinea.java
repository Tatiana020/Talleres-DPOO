...
public class Aerolinea
{
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        if (ruta == null || avion == null) {
            throw new InformacionInconsistenteException("Ruta o avión no encontrados");
        }
        for (Vuelo vuelo : vuelos) {
            if (vuelo.getFecha().equals(fecha) && vuelo.getAvion().equals(avion)) {
                throw new InformacionInconsistenteException("El avión ya está programado para un vuelo en esa fecha");
            }
        }
        Vuelo nuevoVuelo = new Vuelo(avion, ruta, fecha);
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
        ...
        int tiquetesDisponibles = vuelo.getCapacidad() - vuelo.getTiquetes().size();
        if (cantidad > tiquetesDisponibles) {
            throw new VueloSobrevendidoException("No hay suficientes tiquetes disponibles para la venta");
        }
        Cliente cliente = clientes.get(identificadorCliente);
        if (cliente == null) {
            throw new Exception("Cliente no encontrado");
        }
        int valorTotal = cantidad * vuelo.getPrecioTiquete();
        for (int i = 0; i < cantidad; i++) {
            Tiquete nuevoTiquete = new Tiquete(cliente, vuelo);
            vuelo.addTiquete(nuevoTiquete);
        }
        return valorTotal;
    }
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
        ...
        for (Vuelo vuelo : vuelos) {
            for (Tiquete tiquete : vuelo.getTiquetes()) {
                if (tiquete.getCliente().equals(cliente) && !vuelo.isRealizado()) {
                    saldoPendiente += vuelo.getPrecioTiquete();
                }
            }
        }
        return "El saldo pendiente del cliente " + cliente.getNombre() + " es $" + saldoPendiente;
    }
    }
